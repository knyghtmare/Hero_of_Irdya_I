...
-- The vampiric feeding event code
on_event("die", function()
	...
	if not u_killer or not u_killer:matches { ability = "vampiric_feeding" } then
		return
	end
	...
	for i,v in ipairs(wml.get_child(u_killer_cfg, "modifications"))do
		if v[1] == "object" and v[2].vampiric_feeding == true then
